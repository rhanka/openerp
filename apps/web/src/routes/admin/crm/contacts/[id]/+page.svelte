<script lang="ts">
  ...
  import type { Contact, ContactStatus } from "@sentropic/openerp-domain/crm";
  ...
  const contact: Contact | null = $derived(data.contact);

  function statusLabel(status: ContactStatus): string {
    return t(locale, `crm.contacts.status.${status}`);
  }
  function statusTone(status: ContactStatus): "success" | "neutral" {
  ...
  function entryLabel(entryType: string): string {
    ...
    return t(locale, `crm.contacts.entryType.${verb}`);
  }
...
<section class="page">
  <header class="page__header">
    <div>
      <h1>{contact?.displayName ?? t(locale, "crm.contacts.detail.notFound")}</h1>
      {#if contact?.title}
        <p class="page__lede">{contact.title}</p>
      {/if}
    </div>
    <div class="page__actions">
      ...
      <a class="page__back" href="/admin/crm/contacts">
        ← {t(locale, "crm.contacts.detail.action.back")}
      </a>
    </div>
  </header>
  ...
  {#if data.source === "not_found" || !contact}
    <EmptyState
      title={t(locale, "crm.contacts.detail.notFound")}
  ...
  {:else}
    <Card>
      <div class="page__meta">
        <div>
          <dt>{t(locale, "crm.contacts.field.status")}</dt>
          <dd>
        ...
        {#if contact.companyId}
          <div>
            <dt>{t(locale, "crm.companies.field.displayName")}</dt>
            <dd>
              <a href="/admin/crm/companies/{contact.companyId}">{contact.companyId}</a>
            </dd>
          </div>
        {/if}
      </div>
    ...
    <h2 class="page__section-title">
      {t(locale, "crm.contacts.detail.timeline.title")}
    </h2>

    {#if data.timeline.length === 0}
      <EmptyState
        title={t(locale, "crm.contacts.detail.empty.title")}
        message={t(locale, "crm.contacts.detail.empty.message")}
      />
    {:else}
      <ol class="page__timeline" data-testid="crm-contact-timeline">
