...
import {
  ...
  type RunDueScheduledWorkflowsResult
} from "../../src/ticks/workflow.js";
...
describe("tickWorkflow", () => {
  it("applies scope before calling runDue", async () => {
    ...
    await tickWorkflow({ db, organizationId: "org-1", actorUserId: "user-1", runDue });
  ...
  it("calls runDue with the correct tenant and db", async () => {
    const { db } = makeDb();
    const fakeResult: RunDueScheduledWorkflowsResult = {
      processed: 2,
      succeeded: 2,
      failed: 0,
      skipped: 0,
    ...
    expect(calledTenant).toEqual<WorkerTenantContext>({ organizationId: "org-2", actorUserId: "user-2" });
  ...
  it("forwards asOf to runDue", async () => {
    ...
    const runDue = vi.fn().mockResolvedValue({
      processed: 0, succeeded: 0, failed: 0, skipped: 0, asOf: new Date()
    });
    const asOf = new Date("2026-06-01T12:00:00Z");

    await tickWorkflow({ db, organizationId: "org-3", actorUserId: "user-3", asOf, runDue });
  ...
  it("releases scope even when runDue throws", async () => {
    ...
    await expect(
      tickWorkflow({ db, organizationId: "org-4", actorUserId: "user-4", runDue })
  ...
  it("sets the correct organizationId in the scope query", async () => {
    ...
    await tickWorkflow({ db, organizationId: "org-uuid-456", actorUserId: "user-1", runDue });
