...
import { createPgPool, type PgPoolHandle } from "../db/pg-client";
...
const ORG_SLUG = "northwind-services";
const ADMIN_EMAIL = "alice@northwind.local";
...
async function seed(client: Awaited<ReturnType<PgPoolHandle["withClient"]>> extends Promise<infer _T>
  ? never
  : Parameters<Parameters<PgPoolHandle["withClient"]>[0]>[0]): Promise<SeedResult> {
  ...
  const orgRes = await client.query<{ id: string }>(
    `insert into organizations (
     ...
     returning id`,
    ["Northwind Services Inc.", "Northwind Services", ORG_SLUG]
  );
  ...
  await client.query(
    ...
    [userIdentityId, organizationId, ADMIN_EMAIL]
  );
  ...
  // 12 audit events spread over 7 days.
  ...
  // 3 approval_requests: 2 pending, 1 decided.
  await client.query(
